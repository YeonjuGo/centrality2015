...
// ===============================================================================================
    ...
    TH1D* data_pf_energy[5];
    TH1D* data_rec_energy[5];
    TH1D* mc_pf_energy[5];
    TH1D* mc_rec_energy[5];
    TH1D* effi_pf_energy[5];// effi = data/mc
    TH1D* effi_rec_energy[5];// effi = data/mc
    for(int i=0; i<Ncut; i++)
    {
        data_pf_energy[i] = new TH1D(Form("data_pf_energy%d",i), ";pfTowers/energy;Normalized Events",100,0,100);
        data_pf_energy[i] -> SetMarkerStyle(20);
        data_pf_energy[i] -> SetMarkerSize(0.7);
        data_pf_energy[i] -> SetMarkerColor(21+i);
        data_pf_energy[i] -> SetLabelSize(0.03);

        mc_pf_energy[i] = new TH1D(Form("mc_pf_energy%d",i), ";pfTowers/energy;Normalized Events",100,0,100);
       // mc_pf_energy[i] -> SetMarkerStyle(24+i);
       // mc_pf_energy[i] -> SetMarkerSize(0.7);
        mc_pf_energy[i] -> SetMarkerColor(21+i); //marker color
        mc_pf_energy[i] -> SetLineColor(21+i); //line color
        mc_pf_energy[i] -> SetLabelSize(0.03);
   
        data_rec_energy[i] = new TH1D(Form("data_rec_energy%d",i), ";rechitTowers/energy;Normalized Events",100,0,100);
        data_rec_energy[i] -> SetMarkerStyle(20);
        data_rec_energy[i] -> SetMarkerSize(0.7);
        data_rec_energy[i] -> SetMarkerColor(31+i);
        data_rec_energy[i] -> SetLabelSize(0.03);

        mc_rec_energy[i] = new TH1D(Form("mc_rec_energy%d",i), ";rechitTowers/energy;Normalized Events",100,0,100);
       // mc_rec_energy[i] -> SetMarkerStyle(24+i);
       // mc_rec_energy[i] -> SetMarkerSize(0.7);
        mc_rec_energy[i] -> SetMarkerColor(31+i); //marker color
        mc_rec_energy[i] -> SetLineColor(31+i); //line color
        mc_rec_energy[i] -> SetLabelSize(0.03);
        
        effi_pf_energy[i] = new TH1D(Form("effi_pf_energy%d",i), ";pfTowers/energy;DATA/MC",100,0,100);
        effi_pf_energy[i] -> SetMarkerStyle(20);
        effi_pf_energy[i] -> SetMarkerSize(0.7);
        effi_pf_energy[i] -> SetMarkerColor(21+i);
        effi_pf_energy[i] -> SetLabelSize(0.03);
        effi_pf_energy[i] -> SetAxisRange(0.5,1.5,"Y");

        effi_rec_energy[i] = new TH1D(Form("effi_rec_energy%d",i), ";rechitTowers/energy;DATA/MC",100,0,100);
        effi_rec_energy[i] -> SetMarkerStyle(20);
        effi_rec_energy[i] -> SetMarkerSize(0.7);
        effi_rec_energy[i] -> SetMarkerColor(31+i);
        effi_rec_energy[i] -> SetLabelSize(0.03);
        effi_rec_energy[i] -> SetAxisRange(0.5,1.5,"Y");
...
//    datat_pfTower -> Draw("et >>+ data_pf_energy0",eventCut &&  "HLT_HIMinBiasHfOrBSC_v1==1");
//    data_pf_energy[0] = (TH1D*)gDirectory->Get("data_pf_energy0");
/*    datat_pfTower -> Draw("et >>+ data_pf_energy1",eventCut &&  "HLT_HIMinBiasHfOrBSC_v1==1 && pprimaryVertexFilter==1");
    data_pf_energy[1] = (TH1D*)gDirectory->Get("data_pf_energy1");
    datat_pfTower -> Draw("et >>+ data_pf_energy2",eventCut &&  "HLT_HIMinBiasHfOrBSC_v1==1 && phltPixelClusterShapeFilter==1");
    data_pf_energy[2] = (TH1D*)gDirectory->Get("data_pf_energy2");
    datat_pfTower -> Draw("et >>+ data_pf_energy3",eventCut &&  "HLT_HIMinBiasHfOrBSC_v1==1 && phfCoincFilter3==1");
    data_pf_energy[3] = (TH1D*)gDirectory->Get("data_pf_energy3");
  */  datat_pfTower -> Draw("e >>+ data_pf_energy4",eventCut &&  "HLT_HIMinBiasHfOrBSC_v1==1 && pcollisionEventSelection==1");
    data_pf_energy[4] = (TH1D*)gDirectory->Get("data_pf_energy4");
    cout << "data_pf_energy finished"<< endl;

//    mct_pfTower -> Draw("et >>+ mc_pf_energy0");
//    mc_pf_energy[0] = (TH1D*)gDirectory->Get("mc_pf_energy0");
/*    mct_pfTower -> Draw("et >>+ mc_pf_energy1","pprimaryVertexFilter==1");
    mc_pf_energy[1] = (TH1D*)gDirectory->Get("mc_pf_energy1");
    mct_pfTower -> Draw("et >>+ mc_pf_energy2","phltPixelClusterShapeFilter==1");
    mc_pf_energy[2] = (TH1D*)gDirectory->Get("mc_pf_energy2");
    mct_pfTower -> Draw("et >>+ mc_pf_energy3","phfCoincFilter3==1");
    mc_pf_energy[3] = (TH1D*)gDirectory->Get("mc_pf_energy3");
 */   mct_pfTower -> Draw("e >>+ mc_pf_energy4","pcollisionEventSelection==1");
    mc_pf_energy[4] = (TH1D*)gDirectory->Get("mc_pf_energy4");
    cout << "mc_pf_energy"<< endl;

//    datat_recTower -> Draw("et >>+ data_rec_energy0",eventCut &&  "HLT_HIMinBiasHfOrBSC_v1==1");
//    data_rec_energy[0] = (TH1D*)gDirectory->Get("data_rec_energy0");
/*    datat_recTower -> Draw("et >>+ data_rec_energy1",eventCut &&  "HLT_HIMinBiasHfOrBSC_v1==1 && pprimaryVertexFilter==1");
    data_rec_energy[1] = (TH1D*)gDirectory->Get("data_rec_energy1");
    datat_recTower -> Draw("et >>+ data_rec_energy2",eventCut &&  "HLT_HIMinBiasHfOrBSC_v1==1 && phltPixelClusterShapeFilter==1");
    data_rec_energy[2] = (TH1D*)gDirectory->Get("data_rec_energy2");
    datat_recTower -> Draw("et >>+ data_rec_energy3",eventCut &&  "HLT_HIMinBiasHfOrBSC_v1==1 && phfCoincFilter3==1");
    data_rec_energy[3] = (TH1D*)gDirectory->Get("data_rec_energy3");
  */  datat_recTower -> Draw("e >>+ data_rec_energy4",eventCut &&  "HLT_HIMinBiasHfOrBSC_v1==1 && pcollisionEventSelection==1");
    data_rec_energy[4] = (TH1D*)gDirectory->Get("data_rec_energy4");

//    mct_recTower -> Draw("et >>+ mc_rec_energy0");
//    mc_rec_energy[0] = (TH1D*)gDirectory->Get("mc_rec_energy0");
 /*   mct_recTower -> Draw("et >>+ mc_rec_energy1","pprimaryVertexFilter==1");
    mc_rec_energy[1] = (TH1D*)gDirectory->Get("mc_rec_energy1");
    mct_recTower -> Draw("et >>+ mc_rec_energy2","phltPixelClusterShapeFilter==1");
    mc_rec_energy[2] = (TH1D*)gDirectory->Get("mc_rec_energy2");
    mct_recTower -> Draw("et >>+ mc_rec_energy3","phfCoincFilter3==1");
    mc_rec_energy[3] = (TH1D*)gDirectory->Get("mc_rec_energy3");
  */  mct_recTower -> Draw("e >>+ mc_rec_energy4","pcollisionEventSelection==1");
    mc_rec_energy[4] = (TH1D*)gDirectory->Get("mc_rec_energy4");

    //for(int i=0; i<Ncut; i++){
    int i=4;    
    data_pf_energy[i] -> Scale(1./data_pf_energy[i]->Integral("width"));
        data_rec_energy[i] -> Scale(1./data_rec_energy[i]->Integral("width"));
        mc_pf_energy[i] -> Scale(1./mc_pf_energy[i]->Integral("width"));
        mc_rec_energy[i] -> Scale(1./mc_rec_energy[i]->Integral("width"));
  /*    data_pf_energy[i] -> Scale(1./Npf_dataEntries);
        data_rec_energy[i] -> Scale(1./Nrec_dataEntries);
        mc_pf_energy[i] -> Scale(1./Npf_mcEntries);
        mc_rec_energy[i] -> Scale(1./Nrec_mcEntries);
*/  //  }
    effi_pf_energy[i] -> Divide(data_pf_energy[i],mc_pf_energy[i]);
    effi_rec_energy[i] -> Divide(data_rec_energy[i],mc_rec_energy[i]);
...
/*
    TCanvas *c_data_energy = new TCanvas("c_data_energy", "c_data_energy", 800,400);
    c_data_energy -> Divide(2,1,0);
    c_data_energy -> cd(1);
    data_pf_energy[0] -> Draw("ep");
    data_pf_energy[1] -> Draw("same ep");
    data_pf_energy[2] -> Draw("same ep");
    data_pf_energy[3] -> Draw("same ep");
    data_pf_energy[4] -> Draw("same ep");
    
    c_data_energy -> cd(2);
    data_rec_energy[0] -> Draw("ep");
    data_rec_energy[1] -> Draw("same ep");
    data_rec_energy[2] -> Draw("same ep");
    data_rec_energy[3] -> Draw("same ep");
    data_rec_energy[4] -> Draw("same ep");
 */
    TCanvas *c_mc_energy = new TCanvas("c_mc_energy", "c_mc_energy", 800,400);
    c_mc_energy -> Divide(2,1,0);
    c_mc_energy -> cd(1);
//    mc_pf_energy[0] -> Draw("ep");
/*    mc_pf_energy[1] -> Draw("same ep");
    mc_pf_energy[2] -> Draw("same ep");
    mc_pf_energy[3] -> Draw("same ep");
 */ //  mc_pf_energy[4] -> Draw("same ep");
    
    c_mc_energy -> cd(2);
 //   mc_rec_energy[0] -> Draw("ep");
/*    mc_rec_energy[1] -> Draw("same ep");
    mc_rec_energy[2] -> Draw("same ep");
    mc_rec_energy[3] -> Draw("same ep");
  *///  mc_rec_energy[4] -> Draw("same ep");
    
    TCanvas *c_compare_energy = new TCanvas("c_compare_energy", "c_compare_energy", 400,400);
    c_compare_energy -> SetLogy();

    data_pf_energy[4] -> Draw("ep");
    mc_pf_energy[4] -> Draw("same ehist");
    data_rec_energy[4] -> Draw("same ep");
    mc_rec_energy[4] -> Draw("same ehist");
 
    TLegend *leg = new TLegend(0.7,0.7,0.9,0.9);
    leg->SetFillColor(0);
    leg->SetTextFont(43);
    leg->SetTextSize(15);
    leg->AddEntry(data_pf_energy[4],"pfTower DATA","p");
    leg->AddEntry(mc_pf_energy[4],"pfTower MC","l");
    leg->AddEntry(data_rec_energy[4],"rechitTower DATA","p");
    leg->AddEntry(mc_rec_energy[4],"rechitTower MC","l");
       leg -> Draw();

    TCanvas *c_compare_energy_effi = new TCanvas("c_compare_energy_effi", "c_compare_energy_effi", 400,200);
    effi_pf_energy[4] -> Draw("ep");
    effi_rec_energy[4] -> Draw("same ep");
    c_compare_energy_effi->SaveAs("pdf/tower_energy_effi.pdf");

   leg->Draw();

    c_compare_energy -> SaveAs("pdf/tower_energy.pdf");
    
    TCanvas *c_compare_energy_seperately = new TCanvas("c_compare_energy_seperately", "c_compare_energy_seperately", 1000,400);
    c_compare_energy_seperately -> Divide(2,1);
    c_compare_energy_seperately -> cd(1);
  //  c_compare_energy_seperately1 -> SetLogy();
    data_pf_energy[4] -> Draw("ep");
    mc_pf_energy[4] -> Draw("same ehist");
    leg -> Draw();

    c_compare_energy_seperately -> cd(2);
   // c_compare_energy_seperately2 -> SetLogy();
    data_rec_energy[4] -> Draw("ep");
    mc_rec_energy[4] -> Draw("same ehist");

    c_compare_energy_seperately -> SaveAs("pdf/tower_energy_sep.pdf");
